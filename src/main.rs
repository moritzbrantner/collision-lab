...
fn main() {
    ...
    println!();
    println!("AABB tests eliminated: {:.3}%", experiment.grid_test_reduction_percent());
...
fn parse_config(args: Vec<String>) -> Result<Config, String> {
    ...
    while let Some(flag) = args.next() {
        ...
        match flag.as_str() {
            "--objects" => config.objects = parse(&value()?, &flag)?,
            "--cell-size" => config.cell_size = parse(&value()?, &flag)?,
            "--seed" => config.seed = parse(&value()?, &flag)?,
            "--world-extent" => config.world_extent = parse(&value()?, &flag)?,
            "--half-extent" => config.half_extent = parse(&value()?, &flag)?,
