...
test("Zombie Arena captures the mouse and Escape restores the pause menu", async ({ page }) => {
  ...
  await expect.poll(() => frameNumber(page)).toBeGreaterThan(pausedFrame + 20);
  ...
  const afterRestart = await frameNumber(page);
