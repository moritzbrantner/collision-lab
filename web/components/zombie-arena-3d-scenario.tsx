...
const BUILT_WALL_ID_BASE = 10_000;
const BUILD_DISTANCE = 4.0;
const ALGORITHMS = [
...
const BARRICADE_MATERIAL = new THREE.MeshStandardMaterial({
  color: 0xb45309,
  roughness: 0.86,
});
...
export function ZombieArena3dScenario() {
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.HemisphereLight(0xdbeafe, 0x18181b, 1.7));
    ...
    const render = () => {
      ...
      if (current) {
        updateCameraAndAim(camera, current.player, cameraYawRef, cameraPitchRef, aimRef);
      }
  ...
  const broadPhaseAvoided = useMemo(() => {
    ...
    return Math.max(0, 100 * (1 - snapshot.stats.aabbTests / snapshot.stats.possiblePairs));
  }, [snapshot]);

  const builtBarricades = useMemo(
    () => snapshot?.walls.filter((wall) => wall.id >= BUILT_WALL_ID_BASE).length ?? 0,
    [snapshot],
  );
  ...
  const changeAlgorithm = (nextAlgorithm: string) => {
    ...
    if (!world) return;
    try {
      applySnapshot(world.set_algorithm(nextAlgorithm));
    } catch (reason) {
      setError(String(reason));
    }
  };

  const mutateBarricade = (remove: boolean) => {
    const world = worldRef.current;
    const current = snapshotRef.current;
    if (!world || !current) return;
    const [x, z] = buildTarget(current.player);
    try {
      const raw = remove
      ...
      <div className="grid xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="min-w-0 p-4 sm:p-6">
          <div
            ...
            className={`relative min-h-[34rem] overflow-hidden rounded-2xl border bg-zinc-950 outline-none ${
              buildMode
                ? "cursor-cell border-amber-700/70 focus:border-amber-500"
                : "cursor-crosshair border-zinc-800 focus:border-zinc-500"
            }`}
          >
            <div ref={mountRef} className="absolute inset-0" />
            <div
              className={`pointer-events-none absolute left-1/2 top-1/2 z-10 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border ${
                buildMode ? "border-amber-300/90" : "border-cyan-200/80"
              }`}
            >
              <span
                className={`absolute left-1/2 top-1/2 h-1 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full ${
                  buildMode ? "bg-amber-200" : "bg-cyan-100"
                }`}
              />
            </div>
            {buildMode && (
        ...
        <aside className="border-t border-zinc-800 p-5 xl:border-l xl:border-t-0">
          ...
          <p className="mt-2 text-xs leading-5 text-zinc-600">
            A* owns navigation; this selector swaps only the collision broad phase.
          </p>
          ...
          <p className="mt-2 text-xs leading-5 text-zinc-600">
            {buildNotice ??
              "Builds snap to the navigation grid. Shift-click removes only runtime barricades, never static level geometry."}
          </p>
          ...
          <div className="mt-5 rounded-2xl border border-zinc-800 bg-zinc-900/35 p-4">
            ...
            <p className="mt-3 border-t border-zinc-800 pt-3 text-xs leading-5 text-zinc-600">
              A* searches the deterministic 4-neighbour grid, then string-pulls the result across line-of-sight cells without cutting blocked corners.
            </p>
          </div>
...
function buildTarget(player: PlayerSnapshot): [number, number] {
...
function syncZombies(resources: RenderResources, zombies: ActorSnapshot[]) {
  ...
  for (const zombie of zombies) {
    ...
    if (positions && positions.count > 0) {
      const dx = positions.getX(0) - zombie.position[0];
      const dz = positions.getZ(0) - zombie.position[2];
...
function syncNavigation(resources: RenderResources, snapshot: Arena3dSnapshot) {
  ...
  const blockedSignature = blocked.map(([x, z]) => `${x},${z}`).join(";");
  if (resources.blockedSignature !== blockedSignature) {
    if (resources.blockedMesh) {
      resources.navGroup.remove(resources.blockedMesh);
      resources.blockedMesh.geometry.dispose();
      resources.blockedMesh = null;
    }
    if (blocked.length > 0) {
    ...
    resources.blockedSignature = blockedSignature;
  }
  ...
  for (const path of snapshot.debug.navigation.paths) {
    ...
    const zombie = snapshot.zombies.find((actor) => actor.id === path.zombieId);
    const points = [
      ...(zombie ? [new THREE.Vector3(zombie.position[0], 0.08, zombie.position[2])] : []),
...
function updateActor(group: THREE.Group, position: Vec3, aim: Vec3) {
  ...
  if (horizontal > 0.0001) group.rotation.y = Math.atan2(-aim[0], -aim[2]);
...
function updateCameraAndAim(
  ...
  aimRef.current = [aim.x, aim.y, aim.z];
  ...
  camera.lookAt(target.clone().addScaledVector(aim, 8));
}
...
function disposeResources(resources: RenderResources) {
  disposeGeometryTree(resources.scene);
  resources.pathLines.clear();
  resources.pathSignatures.clear();
  resources.zombies.clear();
  resources.walls.clear();
  resources.bullets.clear();
}

function disposeGeometryTree(root: THREE.Object3D) {
  root.traverse((object) => {
    if (object instanceof THREE.Mesh || object instanceof THREE.Line) object.geometry.dispose();
  });
}

function parseSnapshot(raw: string): Arena3dSnapshot {
  return JSON.parse(raw) as Arena3dSnapshot;
}
